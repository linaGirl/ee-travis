!function(){
	...
	module.exports = new Class({
		...
		, init: function(options) {
			...
			try {
				this.travisConfig = require(project.root+'travis.js');
			} catch(err){}
		...
		, get: function(key, dflt){
			return this.travisConfig && this.travisConfig[key] ? this.travisConfig[key] : process.env[key] ? process.env[key] : dflt;
		}
			...
			if (this.publicKey) callback(null, this.publicKey);
				...
				request.get(url, function(err, response, body){
					...
					if (err) callback(err);
						...
						if (!responseData || !responseData.key) callback(new Error('Failed to download public key from «'+url+'»: '+err).setName('DownloadException'));
						else {
							this.publicKey = responseData.key;
